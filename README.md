...
- ⚡ **キャッシュ設計** — 解析結果・変換画像・HLSセグメントをディスクにキャッシュし、上限サイズで自動整理
...
これだけです。初回起動時に `FrameDeck_venv/` へ依存パッケージ
(FastAPI / Uvicorn / Pillow / rarfile など)が自動インストールされ、
ブラウザで `http://127.0.0.1:9000` が開きます。
...
| ツール | 用途 | なくても動く? |
|---|---|---|
| **ffmpeg** | 動画のトランスコード / HLS / サムネイル | ✅ 直接再生できる形式はそのまま再生 |
| **unrar** | RAR / CBR アーカイブの展開 | ✅ ZIP / CBZ はPythonのみで動作 |
