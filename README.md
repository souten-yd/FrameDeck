...
- 🗂 **ライブラリ管理** — 複数ルート登録、★評価、絞り込み、更新日/名前/評価の昇順・降順ソート、
  複数選択による一括削除(★2以下の抽出・重複候補の抽出つき)、ゴミ箱への安全な削除
- 📱 **モバイル最適化** — タップでページ送り、スワイプ、長押し高速送り、2行コントロール、一覧から直接★評価
- 🖱 **PCの操作性** — 一覧の幅をドラッグで調整/最小化(Ctrl+B)、マウスの戻る/進むで前後のファイルへ
  (Windows/Ubuntu どちらでも動作)
- 🔖 **読書位置の記憶** — ページ・再生位置・表示設定・**最後に開いたフォルダ**をサーバ側で保存、端末をまたいで再開
...
- ブラウザが直接再生できる形式は **Range対応ストリーミング** でそのまま配信
  (ETag/Last-Modified付きなのでブラウザのメディアキャッシュが効き、シークが速い)
