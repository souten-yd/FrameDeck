...
## 漫画: レターボックス対応トリミング・見開き分割・画質/速度改善 (2026-07-12)

実例(2.5次元の誘惑 第04巻: 3840x2160スクリーンショット形式、左右と
綴じ目が濃灰色 RGB(51,51,51)、ページは上下端に接する)で灰色トリミングと
見開き分割が失敗していた件への対応。

### 原因
1. 背景色推定が「辺全体のバンド」だったため、上下端がページ内容に汚染
   され colored_or_unknown 判定 → トリミング不発。
2. 見開き検出のアスペクト評価が線形 ((a-1.35)/0.75) で、トリミング後の
   典型的な見開き比率(~1.416)にほぼ0点を与え、しきい値0.68を割った。
3. 分割機構(split_side / split_virtual_pages)は存在したがUI/エンジンに
   未配線で、単ページモードでも横長画像が丸ごと表示されていた。
4. 検出はフル解像度でPythonループ走査しており、1ページ数秒かかっていた。

### 対応
- crop_detector: 四隅パッチ優先の背景推定(四隅が一致すれば採用、不一致
  なら従来の辺バンドへフォールバック)。検出は最長1280pxへ縮小した画像で
  行い、座標を原寸へ戻す(誤差はsafety_marginに吸収)。~150ms/頁。
- spread_detector: 幅1000pxへ縮小して検出。スコアは綴じ目明瞭度0.45/
  バランス0.20/分割位置0.20/縦横比0.15へ再配分。
- 近傍合議 analyze_page_stable: 同一寸法の前後ページ(最大4枚、端では
  片側)と crop_box / split_x の中央値で安定化。自ページの検出失敗は
  近傍が一致する場合のみ救済。見開き/単ページ混在に備え、自ページと
  整合しない近傍は使わず、救済はトリミング後の縦横比が見開き相当の
  ときに限る。表示経路(render)はキャッシュ済み近傍のみで合議し、
  未解析分はバックグラウンドへ(初回表示を待たせない)。プリフェッチが
  解析も先回りする。
- 単ページモードの見開き分割: セッションに half(0/1) を導入し、
  横長ページ(トリミング後の縦横比で判定)は 右→左(RTL) の順で半分ずつ
  表示。ComicViewState.visible_page_sides で面をクライアントへ通知し、
  page/{i}?split_side= で配信。分割位置は合議 split_x(なければ中央)。
  ページ番号・シークは物理ページ基準のまま。
  設定 comic_split_spread_in_single_mode(既定on) と PATCH options で切替。
- 画質: mobileプロファイル dpr上限1.25→2.0 / q76→80、balanced dpr→2.0。
  縮小後に UnsharpMask(radius1.2, percent70) を適用してから圧縮
  (設定 comic_variant_sharpen、既定on)。
- GPUは不採用: 支配的コストはJPEGデコードとNAS読込で、検出の縮小化と
  プリフェッチ並列(ThreadPool)で十分短縮できたため。

### 検証(実アーカイブ + pytest 119件)
- crop_box が全ページ (391,0,3449,2160) で一致(ばらつきゼロ)。
- 単独検出が失敗する表紙/色ページ/暗ページ(0,1,10,40,91)も合議で
  split_x=1914〜1920 に救済。
- 単ページモード: 右→左→次ページ右…、後退は左→右→前ページ左…を
  API/実UI(Playwright mobile)で確認。半分画像は帯・綴じ目なし。
- コールド計測: セッション作成0.1s、初回ページ~1.8s(従来は検出だけで
  数秒/頁)、キャッシュ後5ms。
- モバイル画質: 487px→780px配信 + シャープ化で文字の輪郭が明瞭に。

## キャッシュ容量の自動整理 + README整備 (2026-07-12)

- READMEに必要環境(python3/python3-venv/pip 必須、ffmpeg/unrar 任意、
  デスクトップUIは python3-tk/mpv)とaptインストール例を追記。
- 漫画ディスクキャッシュ(変換画像/ページ/サムネイル)にサイズ上限を導入:
  `comic_cache_max_mb`(既定100MB、0で無制限)。超過時は古い順に上限の
  8割まで削除(ヒステリシス)。キャッシュヒット時に mtime を更新して
  実質LRU。走査は os.scandir のみで軽量、実行は
  ①起動時 ②キャッシュ書き込み後(10分に1回まで、スレッドプールで
  バックグラウンド) ③設定変更直後。解析JSONは小さく再計算コストが
  高いため対象外。
- 動画HLSキャッシュの上限設定を `video_variant_cache_gb`(30GB) から
  `video_variant_cache_mb`(既定300MB) へ変更。設定UIも両方とも
  数値入力で変更可能(「漫画キャッシュMB」「動画キャッシュMB」)。
- 検証: 設定APIで上限を2MBへ変更→2秒以内に3.7MB→1.6MBへ削減、
  最古ファイルから削除されることを実機確認。pytest 120件全通過。

### 追補: 回転ロックの表示崩れ修正 (2026-07-12)

報告: 横向きでロック後に端末を縦向きへ戻すと固定されず表示が崩れる
(縦向きロックは正常)。

原因: 回転フォールバックをメディアクエリCSSで行っていたが、全画面表示
(.viewer.fullscreen-active)の width/height/inset が !important 指定の
ため回転用の寸法が上書きされ、transformだけ適用されて崩れていた。
横向き視聴では全画面を併用しやすく、横向きロックで顕在化した。
メディアクエリ+dvh/dvw依存も端末差の温床だった。

対応: 回転の適用をJSのインラインstyle(priority "important" =
スタイルシートの!importantより優先)へ変更し、寸法は
innerWidth/innerHeight から直接計算。物理向きがロックと一致する間は
インラインを外して基本CSS(fixed全面)に戻す。メディアクエリの回転
ブロックは削除。resize時は既存ハンドラが再適用する。

検証: Playwrightで6ケース(横ロック→縦回転→復帰、横ロック+全画面→
縦回転、縦ロック→横回転、解除)すべて画面充填+期待どおりの回転を確認。
pytest 120件全通過。

### 追補2: 実機で回転ロックが効かなくなる回帰の修正 (2026-07-12)

報告: 追補1の修正後、実機で縦ロック→横回転・横ロック→縦回転とも
ロックが効かなくなった。

原因: 回転適用をJS(resizeイベント)駆動へ変えたことによるモバイル特有の
タイミング問題。実機の回転直後は resize 発火時点で
innerWidth/innerHeight がまだ旧向きの値を返すことがあり(特にiOS)、
「物理向き=ロック方向」と誤判定して回転を外したまま以後のイベントが
来ず放置される。旧CSSメディアクエリ方式はブラウザが常時評価するため
起きなかった(エミュレータでは寸法が同期更新されるため再現せず)。

対応:
- 向き判定を matchMedia("(orientation: landscape)") ベースへ変更
  (CSSエンジンと同一の判定)。
- 回転寸法をピクセル値から 100dvh/100dvw(未対応環境はvh/vw)の
  ビューポート単位インラインstyleへ変更し、確定前のJS読取値に
  依存しない形にした。
- resize に加えて orientationchange / matchMediaのchange でも再評価し、
  さらに rAF + 350ms後 の遅延再適用で確定後の状態に自己修復する
  (refreshVideoOrientationLock)。

検証: 6ケース(横/縦ロック×回転×全画面併用)全通過に加え、
「誤って回転が外れた状態」を再現し orientationchange で復元されることを
確認。pytest 120件全通過。

### 追補3: 回転時の「ぐるん」エフェクトの抑制 (2026-07-12)

報告: 回転ロックは保持されるようになったが、端末の向き変更時に一瞬
回転アニメーションが見える。

原因: OS/ブラウザの画面回転アニメーション + 回転補正の適用タイミング差。
回転自体は消せないため、切替の間だけ隠す方式とした。

対応: 向きの変化を検知した瞬間にプレーヤーを opacity:0 (!important、
transitionなしで即時)にし、回転確定(380ms)後に補正を適用してから
0.15秒のフェードで復帰(refreshVideoOrientationLock内)。
向きが変わらないresize(ツールバー開閉等)ではマスクせず点滅させない
(lastSeenOrientationで判定)。解除時はマスク関連のインラインを掃除。

検証: 回転直後 opacity=0 → 確定後 1 + 回転適用、残留インラインなし、
同一向きresizeで点滅なし、既存6ケース回帰なし。pytest 120件全通過。

## ライブラリ操作の刷新と動画配信の安定化 (2026-07-31)

### 1. 並び順(フォルダが日付順にならない)

`list_items` は `files` だけを日付で並べ替え、`folders` は `list_folder`
の自然順(名前順)のままだったため、「日付順」でもフォルダは名前順で
並んでいた。フォルダ・ファイルを同一規則で並べる `_sorted_items()` に
統一し(フォルダを先頭に置く点は維持)、並び順キーを
`date_desc / date_asc / name_asc / name_desc / rating_desc / rating_asc`
の6種へ拡張した。旧クライアント互換のため `date` / `name` も受け付ける。
選択した並び順・絞り込みは localStorage に保存する。

### 2. モバイルで★が付けられない

モバイルでは topbar の star-bar がメディアクエリで非表示になり、
評価する手段が無かった。
- ドロワー内に `#star-bar-mobile`(★44px相当のタップ領域)+ 削除ボタンを追加
- 一覧行の★をタップすると評価シート(モーダル)が開く。ファイルを開かずに
  評価できるため、モバイルでもPCでも操作数が最小になる
- 低い画面高さ(横向き等)でツール群が一覧を押し出さないよう、
  ツール側を `flex: 0 1 auto; min-height: 0; overflow-y: auto`、
  一覧を `flex: 1 0 42dvh` にした

### 3. 一括削除と重複抽出

- 複数選択モード(`☑ 選択`)を追加。チェックボックス選択、★2以下の一括選択、
  全選択/解除、選択件数表示、確認ダイアログ(対象名を列挙)つき一括削除。
- 一括削除も既存の確認トークン方式を使う。トークンの subject は対象ID集合
  (`bulk-delete:<sorted ids>`)にしており、確認後に対象を差し替えられない。
- 重複抽出 `GET /api/library/duplicates` は
  **拡張子と評価タグを除いた表示名を数字込みで比較し、編集距離 ≤ 2**(既定)
  かつ **名前に含まれる数値が一致** するものを同一視する。
  グループ内で更新日時が最も新しいものを「残す」、それ以外を選択候補にする。
  拡張子は判定対象(違っても重複)だが文字数の差には数えない。
  数値一致を条件にしたのは、数字を無視すると第01巻/第02巻が重複扱いになり、
  そのまま一括削除すると別の巻を消してしまうため
  (例: `..._第01巻.zip` / `..._第01巻.rar` / `..._第01巻s.zip` は同一、
  `..._第02巻.zip` は別物)。
  誤検出時に取り返しがつかないため **選択するだけで削除はしない**。
  UI側も一覧(残す/選択+更新日)を確認してから適用する2段階にした。
  比較は長さ順+打ち切り付きLevenshtein(`bounded_distance`)でO(n²)を実用域に。

### 4. 一覧ペインの最小化・幅変更 (PC)

一覧の上に常設ツールバー(最小化 ⟨ / 既定に戻す ⤢ / 件数 / 選択)を置き、
右端にドラッグ用ハンドルを追加した。幅と最小化状態は localStorage に保存。
ダブルクリックで既定幅、←→キーでも調整可、Ctrl+B で最小化トグル。
幅は CSS変数 `--library-width` で持つ(インラインstyleにするとモバイルの
ドロワー幅指定を上書きしてしまうため)。

### 5. マウスの戻る/進む (Ubuntu対応)

Windowsのブラウザはサイドボタンを button=3/4 のマウスイベントとして
配送するが、Ubuntuではブラウザが履歴移動として消費しページに届かない。
そこで二段構えにした:
- マウスイベントが届く環境: 従来どおり preventDefault して処理
- 届かない環境: 履歴を `[前][現在][次]` の3件にしておき、popstate で
  戻る/進むを検知してメディア移動に割り当て、中央へ復帰させる
ビューアを開いていない時は復帰させないので、通常の離脱も可能。
設定 `aux_mouse_navigation` で無効化できる。

### 6. Wi-Fi/有線は原寸、モバイル回線のみ1080p

`classify_network()` を追加し、`navigator.connection.type` と
**クライアントIPがプライベート帯か**(Network Information API 非対応の
Safari等の代替シグナル)から lan / cellular / slow / unknown を判定する。
`video_profile_desktop` / `video_profile_mobile` の既定を `auto`
(=回線で判定)に変更し、LANなら原寸、モバイル回線は
`video_cellular_max_resolution`(既定1080p)を上限にする。
直接再生できない形式(HEVC/MKV等)は「原寸のまま変換」(スケーリングなし)
で配信する。上限内の動画は再変換せず直接再生する。
既存 settings.json は `settings_version` による移行で旧既定値
(1080p/720p/original)を `auto` へ寄せる。

### 7. 再生が止まり再起動が必要になる不具合

原因は複数あり、いずれも「連続操作でサーバ側の変換が積み上がる」ことに
帰着した。
1. **fMP4変換プロセスが残る**: StreamingResponse に同期ジェネレータを
   渡していたため、クライアント切断時に生成側の finally が呼ばれず
   ffmpeg が残る。シーク連打の回数だけ ffmpeg が同時に走り、CPUを
   食い潰して再生が進まなくなる。
   → 送出を async ジェネレータ化(切断時に必ず `close()`)し、
   タブ単位のセッションIDで**同一タブの古い変換を必ず停止**する。
   送出が一度も始まらなかったストリームは回収スレッドが片付ける。
2. **ffmpegのstderrパイプ詰まり**: `stderr=PIPE` を誰も読まないため、
   警告が多い入力では64KBで書き込みブロック→配信停止。一時ファイルへ変更。
3. **スレッドプール枯渇**: HLSセグメント待ち(最大10秒)を同期defで
   行っていたため、既定40スレッドを占有し、一覧・設定・漫画ページまで
   無応答になっていた。待機を async 化し、配信系は専用の
   CapacityLimiter(24)を使う。既定プールも96へ拡張。
4. **HLSジョブの多重起動**: `cancel_source` → `_register_job` が並行実行
   されると互いを取り逃がす。開始処理を `_start_lock` で直列化した。
5. **クライアント側**: 変換のシークは320msデバウンスしてから作り直す。
   `openVideo` に世代番号を持たせ、古い応答が新しい再生を上書きしない
   ようにした。再生位置が進まない状態が12秒続いたら同じ位置から自動で
   読み直す(ウォッチドッグ)ため、再起動なしで復帰する。

検証: 実サーバに対し、同一セッションでの変換25連打 → 稼働ffmpegは常に1本、
再生は継続。HLS 12連打 → 停止要求されていないジョブは1本。配信12本+HLS12本
の最中でも `/api/health` 3ms・`/api/library/items` 50msで応答。
切断後の残存ffmpegは0。pytest 151件全通過。

## 原寸再生のカクつき解消と、前回フォルダからの再開 (2026-07-31 追補)

### 1. 原寸で再生するとカクつく

報告: 原寸で視聴するとひどくカクつくことがある。プレーヤーで直接開いても
そうはならない。NAS→配信サーバ→クライアントの多段が原因では?

調査:
- NAS(CIFS //192.168.68.57)からの実測読み出しは **93〜104 MB/s**。
  対象ファイルは 6〜32 Mbps(≒4 MB/s)なので、転送経路は律速ではない。
- 実際の原因は **不要なトランスコード**。`probe.py` の
  `_BROWSER_VIDEO_CODECS` に hevc が無く、HEVCは常に「直接再生不可」と
  判定されていた。一方このChromeは `canPlayType('video/mp4;
  codecs="hvc1.2.4.L153.B0"')` に **probably** を返す(=再生できる)。
  そのため 3840x2160/60fps/32Mbps のHEVCを実時間でH.264へ再エンコード
  しようとして破綻していた(libx264 veryfast でも到底間に合わない)。
  以前は `video_stream_mode=original` だったため変換自体が起きず、
  この設定を auto へ移行した時点で顕在化した。

対応:
- **再生可否の判定をクライアント申告に切り替えた**。ブラウザ側で
  `canPlayType` を用いて映像/音声/コンテナの対応表を作り
  (`clientCodecSupport`)、`playback-profile` へ送る。サーバは
  `resolve_direct_play()` でこの申告を優先し、申告が無い場合のみ
  従来のサーバ側推定にフォールバックする。
  判定に `MediaSource.isTypeSupported` は使わない。MSE経由の可否は
  `<video src>` の直接再生可否と一致しない(Chromeはmpeg-tsをMSEでは
  扱えるがファイル直再生はできない)。
- **コンテナだけが非対応なら再エンコードせず remux する**。
  MKVのH.264などは `-c:v copy`(必要なら音声のみAAC化)でmp4へ入れ替える。
  `build_fmp4_transcode_cmd(copy_video=…, copy_audio=…)`。
- **本当に再エンコードが要る場合は実時間で処理できる解像度まで下げる**
  (`REALTIME_ENCODE_MAX=1080p`)。4K60を原寸で実時間エンコードするのは
  不可能で、カクつく再生を返すより縮小したほうが実用的なため。
  バッジに「実時間変換のため縮小」と表示する。
- ブラウザの申告が外れて直接再生に失敗した場合は、`video.error` を捕えて
  自動的にremux配信へ切り替える(`fallbackToTranscode`、1回だけ)。

検証: 3840x2160/60fps/HEVC(38GB)を実機Chromeで再生し、
15.2秒の実時間に対して再生も15.2秒進行、**ドロップフレーム0**、
先読みバッファ17秒、ffmpegプロセスは0本(=変換なし)。
H.264 1080pも直接再生でドロップ0。MKV(H.264+AC3)は
「原寸そのまま配信 (matroska → mp4)」として映像無変換で再生。

### 2. 前回開いていたフォルダから再開する

`GET /api/library/items` が成功するたびに、モード別の最終フォルダを
`settings` テーブル(`last_folder.comic` / `last_folder.video`)へ記録する。
クライアントは起動時に `GET /api/library/start-folder?mode=…` を呼び、
返ってきたフォルダを開く。フォルダが消えている・ルート外・設定が
`root` の場合は null を返し、従来どおりライブラリのルートを開く。

- 設定 `library_start_folder`(既定 `last`)で「前回のフォルダ / ルート」を
  切り替えられる。漫画・動画の両方に適用し、モードごとに別々に記憶する。
- 記録はサーバ側なので、別の端末で開いても同じ場所から再開できる。
- パンくずは `relative_path` のみ表示にした(フォルダ名が二重に出ていた)。

検証: サブフォルダを開いた状態でリロード → 同じフォルダで復帰し
「前回の場所から再開」を表示。設定を root にすると null を返して
ルートから開始。漫画と動画で別々に記憶されることも確認。pytest 159件全通過。

### 追補2: 残るカクつきの正体は「画面と映像のフレーム比」 (2026-07-31)

報告: ETag対応後もまだカクつく(特に FC2PPV-3153960.mp4)。
「ディスプレイのフレームレートを取得して合わせられないか」。

調査:
- 対象は h264/1080p/**29.97fps CFR**/9Mbps。フレーム間隔は33.4msで完全に一定、
  faststart、サーバ側スループットは111MB/s。配信・復号ともに問題なし。
- 一方 **ディスプレイは 3440x1440 @ 84.91Hz** だった(EDIDの詳細タイミングでも
  84.98Hz)。84.91 ÷ 29.97 = **2.833 回/コマ**。整数でないため、各フレームは
  3,3,2,3,3,2… 回ずつ表示される。表示時間が 35.3ms / 23.5ms と ±20% 揺れ、
  これが judder(カクつき)として見える。24fps素材を60Hzで見る3:2プルダウンと
  同じ現象で、パン映像で特に目立つ。
- mpv等のプレーヤーが滑らかなのは `--video-sync=display-resample` 相当の
  補正(再生速度を僅かに変えて画面に同期)を行うため。

採らなかった案 — **サーバ側でfpsを変換する**:
ffmpegの `fps` フィルタは目標fpsの格子にフレームを複製するだけで、
84.91Hzのような非整数比では同じ不均等パターンが残る。judderは消えないのに
実時間エンコードのCPU負荷と通信量(1080p85fpsは約3倍)だけ増える。
動き補間(minterpolate)なら滑らかになるが実時間の数百倍遅く実用外。

採用した案 — **端末側で速度を微調整する(display-sync)**:
- `requestAnimationFrame` の間隔の中央値から実測リフレッシュレートを求める。
- 映像fpsとの比 `ratio = hz / fps` を最も近い整数 `k` に丸め、
  `playbackRate = ratio / k` を適用する。ズレが **±1.2% 以内のときだけ**適用し、
  それ以上なら何もしない(速度が体感で変わるため)。音程はブラウザが保持する。
- 例: 59.97Hz + 29.97fps → +0.05% で 1コマ=2回に固定。
  120Hz + 23.976fps → +0.10% で 1コマ=5回。
- 補正で吸収できない場合は原因と対処を通知する
  (「画面 84.9Hz ÷ 映像 29.97fps = 2.83回/コマ」+ 推奨リフレッシュレート)。
- 非表示タブでは rAF が絞られて測定できないため測らない(補正もしない)。
  ウィンドウのリサイズ・復帰、60秒経過で測り直す(別ディスプレイへの移動対応)。
- 設定 `video_display_sync`(既定 auto / off)。

端末別の判定結果(実装のロジックで計算):

| 画面 | 23.976 | 24 | 25 | 29.97 | 30 | 59.94 | 60 |
|---|---|---|---|---|---|---|---|
| 60Hz (一般PC/iPhone) | ✗16.6% | ✗16.7% | ✗20% | +0.10% | 一致 | +0.10% | 一致 |
| 120Hz (ProMotion) | +0.10% | 一致 | ✗4% | +0.10% | 一致 | +0.10% | 一致 |
| 144Hz (Y700等) | +0.10% | 一致 | ✗4% | ✗3.9% | ✗4% | ✗20% | ✗20% |
| 84.91Hz (現行PC) | ✗11.5% | ✗11.6% | ✗13.2% | ✗5.6% | ✗5.7% | ✗41.7% | ✗41.5% |

このPCのパネルはEDID上 3440x1440 で 84.98 / 59.97 / 49.99 Hz を持つ。
**59.97Hzへ切り替えると 29.97/30fps 素材が 1コマ=2回の完全均等**になる
(24fps素材は60Hz系では3:2プルダウンが残る。48/120Hzなら解消)。

### 追補3: 「不定期に」カクつく件 — judderとは別 (2026-07-31)

追加の報告で状況が変わった:
- カクつきは**常時ではなく不定期**。
- **変換配信では問題が出ない**(変換コマンドは `-r` もfpsフィルタも
  使っておらず、**フレームレートは変えていない**)。

これで表示カデンス(3:2プルダウン)説は否定される。同じfpsの変換配信が
滑らかなら、原因はfpsではない。切り分けのために測った事実:

| 調査 | 結果 |
|---|---|
| NASを再生ペースで5分読み続ける | 1MB読みの遅延 p50 0.1ms / p99 0.4ms / 最大0.7ms、**200ms超のスパイク0回** |
| HTTP経由のスループット | 111 MB/s(先読み実装後 140 MB/s) |
| 任意オフセットのRange要求 | 初回応答 2〜81ms |
| 原本と変換出力のトラック構成 | どちらも video/audio/**bin_data** で同じ(データトラックは差分ではない) |

残る差分は次の2点:
1. **配信の形**: 直接再生はブラウザがRangeで細かく取り直す
   (progress/suspendが0.1秒間隔で繰り返されるのを観測)。
   変換配信は1本の連続ストリームで、取り直しが起きない。
2. **映像の性質**: 原本は High profile + Bフレーム2、変換出力は
   Constrained Baseline + Bフレーム0(フレーム並べ替えが不要)。

対応:
- **先読み(readahead)**: 配信を「NASから読む→送る」の交互実行から、
  先読みスレッド+8MBのキューへ変更。共有側が数百ms詰まっても在庫で
  埋められる。スループットも111→140MB/sへ改善。切断時にスレッドが
  残らないことを確認(15回連続切断でスレッド数7→7)。
- **読み出し遅延のログ**: 1チャンクが400msを超えたら警告を記録する。
- **再生の乱れの記録**: バッファ待ち(供給不足)と表示落ち(描画が
  間に合わない)を区別して記録し、設定画面に表示する。原因1と2の
  どちらかを事実で切り分けるため。
- **「原寸(再多重化)」配信の追加**: 画質・コーデックはそのまま
  (`-c copy`)でコンテナだけ作り直し、変換配信と同じ連続ストリームで
  送る。CPUコストは実測4240倍速でほぼ無料。
  これは切り分けの決定打にもなる — 再多重化で直れば原因は配信の形(1)、
  直らなければ復号側(2)と確定する。

**結果**: 再多重化を選ばず、**直接再生(原寸)のままで解消**した。
つまり原因は上記(1)の配信の形 — 正確には「NASから読む」と「送る」を
交互に行っていたことによる供給の揺らぎで、先読みの導入で埋められた。
復号側(High profile + Bフレーム)は無関係だったことになる。
再多重化の経路は残しておく(共有側がさらに遅い環境や、
コンテナ由来の問題が出たときの逃げ道として有効なため)。

### 追補4: モバイル(iOS)の再生経路を1080p固定に (2026-07-31)

実機確認: iOSでは原寸(直接再生)が安定せず、1080pなら問題なく再生できる。

原因の整理:
- 原寸 = ファイルをそのままRange配信。iOS Safariのメディアスタックは
  Wi-Fi越しの長時間Range配信で詰まりやすい。
- 1080p = 変換が必要 → モバイルはHLS経路 → セグメント単位の取得になり、
  取りこぼしや遅延の影響を受けにくい。

対応:
- `video_profile_mobile` の既定を **1080p** にした(回線種別によらない)。
  設定バージョン3で、既存の "auto" は 1080p へ移行する。
- モバイルで上限を設定している間は、素材が上限内でも直接再生へ
  ショートカットしない(`select_video_profile` の `ui_profile != "mobile"` 条件)。
  1080p素材でもセグメント配信で届けるため。
- PCは従来どおり(LANなら原寸・無変換の直接再生)。
- モバイルでも設定で「原寸」を選べば尊重する。

### 追補5: iOSの1080p原寸を安定HLS配信にする (2026-09-01)

iOS実機で、原寸指定時にカクつく素材とカクつかない素材を再調査した。

- 対象は1080p/H.264/29.97fps、平均約6.1Mbpsだが1秒間ピークは
  23.45Mbps。比較素材は同等の平均ビットレートでピーク14.18Mbpsだった。
- 64MiB Range応答は初回でもTTFB 5.8ms、約100MB/s。ディスクは
  65〜84MB/s、映像復号は約50倍速、DTS逆行は0件。サーバーI/O、
  HTTP送出、デコーダ能力は原因から除外した。
- iOS Safariの原寸はRange取得を繰り返すため、瞬間ビットレートと
  Wi-Fiの揺らぎによってバッファが痩せる。ファイルごとに発生が
  異なる理由と一致する。
- 同区間の1080p HLSはマスター即時、最初の2秒プレイリストが0.60秒で
  到達し、素材と同じ寸法のままビットレートを平準化できた。

そのため、Native HLS対応のモバイル端末で1080p以下の「原寸」を
選んだ場合は、解像度を変えず2秒HLS経路を使う。4K/1440p素材は
実時間エンコード余力が保証できないため、従来の原寸経路を維持する。

PCで明示的に「原寸」を選んだ場合も、映像・音声はコピーして
連続fMP4に再多重化する。実測でTTFB 14ms、約91MB/sで、解像度・
ビットレート・fpsを変えずRange取得の揺らぎだけを除いた。「自動」の
Direct Playは従来どおりとし、不要なffmpegプロセスは起動しない。
