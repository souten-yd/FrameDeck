...
class LibraryService:
    ...
    def _ensure_default_roots(self) -> None:
        existing = {r["path"] for r in self._storage.list_roots()}
        for key, kind in (("default_folder_comic", "comic"),
                          ("default_folder_video", "video")):
            path = self._settings.get(key)
            if path and path not in existing and os.path.isdir(path):
                self.add_root(path, kind)
    ...
    def add_root(self, path: str, kind: str = "any") -> dict:
        resolved = str(Path(path).resolve())
        if not os.path.isdir(resolved):
            raise FileNotFoundError(f"フォルダが見つかりません: {path}")
        root_id = media_id_for_path(resolved)
        self._storage.add_root(root_id, resolved, kind)
        return {"id": root_id, "path": resolved, "kind": kind}
