...
class Services:
    def __init__(self, settings: Settings, storage: Storage, paths: AppPaths):
        ...
        self.pipeline.disk_cache_max_bytes = int(
            settings.get("comic_cache_max_mb", 100)) * 1024**2
    ...
    def _on_settings_changed(self, values: dict) -> None:
        ...
        self.pipeline.disk_cache_max_bytes = int(
            values.get("comic_cache_max_mb", 100)) * 1024**2
        ...
        threading.Thread(target=self._prune_hls_safe,
                         name="framedeck-hls-prune", daemon=True).start()

    def _prune_hls_safe(self) -> None:
        try:
            self.hls.prune()
        except Exception:
    ...
    def startup_maintenance(self) -> None:
        ...
        try:
            ...
            logger.exception("HLSキャッシュ整理に失敗しました")
        try:
            self.pipeline.prune_disk_caches()
        except Exception:
            logger.exception("漫画キャッシュ整理に失敗しました")
        try:
